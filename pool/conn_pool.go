...
type ClientPool struct {
	...
	// MaxLiveTime is the max live time after conn first established.
	...
	for {
		if !forceNew {
			...
			for i, n := 0, p.idle.Len(); i < n; i++ {
				...
				test := p.TestOnBorrow
				p.mu.Unlock()
				if test == nil || test(ic.client, ic.t) == nil {
...
func (p *ClientPool) put(c interface{}, firstInit time.Time, forceClose bool) error {
	...
	if !p.closed && !forceClose {
		p.idle.PushFront(&idleConn{t: time.Now(), client: c, pool: p, ft: firstInit})
